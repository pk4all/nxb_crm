                  ...
                  <div class="row"> 
                    ...
                    <div class="col-sm-6">
                      <div class="img mb-3">
                        ...
                        <img :src="'/uploads/form/images/'+form.profileImage" alt="" style="width:20%;">
                      </div>
                      <div class="img mb-3">
                        ...
                        <img :src="'/uploads/form/images/'+form.coverImage" alt="" style="width:40%;">
...
export default {
  ...
  setup() {
      const getIdFromUrl = ()=> {
        ...
        const id = url.split('/').pop(); // Assumes the ID is the last segment of the URL
        return id;
      }
      ...
      const itemId = ref(getIdFromUrl());
      onMounted(async()=>{
        
        if(itemId.value){
          try {
            const response = await fetch('/form/getForm/'+itemId.value);
            if (!response.ok) {
              throw new Error('Network response was not ok');
            }
            const jsonData = await response.json();
            form.value=jsonData;
            fieldSets.value=jsonData.fields;
          } catch (err) {
    ...
    const saveForm = async()=>{
      ...
      if(error.value==''){
        try {
          const response = await axios.post('/form/save-form/'+itemId.value, form.value, {
              headers: {
